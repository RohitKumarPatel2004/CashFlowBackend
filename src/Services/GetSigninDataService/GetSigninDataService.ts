  ...
  Login: async (request: Request, response: Response) => {
    try {
      ...
      if (result.length === 1) {
        ...
        if (isPasswordMatch) {
          ...
          return response.status(200).json({
            success: true,status:200, user, token
          });
        } else {
      ...
      if (error instanceof HttpException) {
        return response.status(error.status).json({ success: false,status:401, error: error.message, errorMessage });
      } else {
        return response.status(500).json({ success: false,status:500, error: "Internal server error", errorMessage });
      }
    }
  },
};
