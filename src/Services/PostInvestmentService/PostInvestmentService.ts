...
export const PostInvestmentService = {
  PostInvestment: async (request: Request, response: Response) => {
    try {
      const { planName, price, dailyProfit, totalRevenue, days } = request.body;
      const query = 'INSERT INTO investment (planName, price, dailyProfit, totalRevenue, days) VALUES (?, ?, ?, ?, ?)';
      const bindValues = [planName, price, dailyProfit, totalRevenue, days];
